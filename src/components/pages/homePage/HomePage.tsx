...
export default function Homepage(props: HomePageCombinedProps) {
  ...
  const coins = props.homePage.coins;
    ...
    <>
      {visibleInformation && (
        <HomeInformation
          ...
          setVisibleInformation={setVisibleInformation}
        />
      )}

      <div className="absolute w-full h-auto pb-[200px] bg-cardmon-gray">
        <StepCounter steps={steps} setSteps={setSteps} coins={coins} setCoins={() => {}} />

        <div className="relative flex flex-col size-full">
          <div className="flex w-full h-8 mt-8 justify-between">
            <button
              className={`w-8 h-8 rounded-full flex justify-center items-center ml-4 ${
                visibleInformation ? 'bg-cardmon-orage' : 'bg-white'
              }`}
              onClick={() => setVisibleInformation(!visibleInformation)}
            >
              <svg
                className={`size-[18px] ${visibleInformation ? 'fill-white' : ''}`}
                viewBox="0 0 18 18"
        ...
        <div className="h-8 w-full mt-8 flex justify-between items-center">
          ...
          >
            <div className="size-4 relative">
              <span
                className={`w-4 h-1 z-20 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 ${
                  visible ? 'bg-white' : 'bg-cardmon-light-gray'
                }`}
              ></span>
              <span
                className={`w-1 h-4 z-10 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 ${
                  visible ? 'bg-white' : 'bg-cardmon-light-gray'
                }`}
              ></span>
            </div>
        ...
        {visible && (
          <CardsMissions
            {...props.cardMissions}
            cardDailySteps={steps}
            cardWeeklySteps={steps}
          />
        )}
      </div>
    </>
